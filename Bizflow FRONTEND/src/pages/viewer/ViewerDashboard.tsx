    ...
    <MainLayout>
      <div className="space-y-6 animate-slide-up">
        <div>
          ...
          <p className="text-neutral-500 text-sm">Business status at a glance</p>
        </div>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Card className="p-4 bg-white border border-coffee-200 shadow-subtle">
              <Clock size={16} className="text-coffee-700 mb-2" />
            ...
            {queue.length > 0 ? queue.map((item, i) => (
              <div key={item.id} className="p-4 flex justify-between items-center hover:bg-coffee-50 transition-standard">
                ...
                <div className={`px-2 py-1 rounded-badge text-[8px] font-black uppercase ${
                   item.status === 'WAITING' ? 'bg-coffee-100 text-neutral-500' : 'bg-coffee-700/10 text-coffee-700'
                }`}>
                  {item.status}
                </div>
              </div>
        ...
        <section className="p-6 bg-coffee-900 text-white rounded-3xl shadow-large relative overflow-hidden">
           <div className="relative z-10">
              <div className="flex items-center gap-2 mb-4">
                 <BarChart2 size={18} className="text-coffee-700" />
              ...
              <div className="space-y-3">
                 ...
                 <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                    <div 
                       className="h-full bg-coffee-700 transition-all duration-1000 shadow-[0_0_10px_rgba(107,62,46,0.3)]" 
           ...
           <div className="absolute -right-10 -top-10 w-32 h-32 bg-coffee-700/10 rounded-full blur-3xl" />
        </section>
      </div>
    </MainLayout>
  );
