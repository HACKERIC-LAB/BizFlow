...
export const MainLayout = ({ children, hideBottomNav }: MainLayoutProps) => {
  return (
    <div className="min-h-screen bg-coffee-50 flex justify-center items-center md:p-8 lg:p-12 relative overflow-hidden">
      {/* Background Gradient Layer */}
      <div className="absolute inset-0 z-0 soft-gradient-bg pointer-events-none" />
      
      {/* Phone Frame for Desktop */}
      <div className="w-full h-screen md:h-[844px] md:max-w-[390px] bg-coffee-50 flex flex-col relative md:shadow-2xl md:rounded-[3rem] overflow-hidden md:border-[12px] md:border-coffee-900 z-10">
        {/* Mock Status Bar - Only on Desktop */}
        <div className="hidden md:flex h-12 bg-coffee-900 items-center justify-between px-8 pt-6 pb-2 text-[12px] font-bold z-50 text-white/80">
          <span>9:41</span>
        ...
        {/* App Content */}
        <div className="flex-1 flex flex-col relative overflow-hidden bg-coffee-50">
          <AppHeader />
          <main className="flex-1 overflow-y-auto pb-40 pt-2 px-5 w-full no-scrollbar relative z-10">
            {children}
          </main>
          <AIChatFAB />
